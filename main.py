...
class Tornado(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y):
        self.pos_x = pos_x
        self.pos_y = pos_y
        super().__init__(tornado_group, all_sprites)
        self.image = torimage
        self.rect = self.image.get_rect().move(
            tile_width * pos_x + 15, tile_height * pos_y + 5)
    ...
    for y in range(len(level)):
        for x in range(len(level[y])):
            if level[y][x] == '.':
                ...
                Tile('empty', x, y)
                torn = Tornado(x, y)
    return new_player, x, y, new_evil, star, torn
...
def rand_screen():
    intro_text = ["RANDOMIZER", "",
                  "Вы попали в портал, ",
                  "мы вынуждены переместить вас в любую точку сюжета"]
    fon = pygame.transform.scale(load_image('fon.jpg'), (WIDTH, HEIGHT))
    screen.blit(fon, (0, 0))
    font = pygame.font.Font(None, 50)
    text_coord = 50
...
torimage = load_image('arrow.png', -1)
...
player, level_x, level_y, ev, st, tr = generate_level(load_level('map.txt'))
